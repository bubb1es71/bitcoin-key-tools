...
- Sensitive intermediate values are wiped with [`zeroize`](https://crates.io/crates/zeroize). The main owned secrets (`rolls`, `entropy`, `passphrase`, `phrase`, and `seed`) are wrapped in `Zeroizing`, so they are zeroized automatically when they go out of scope. The BIP39 mnemonic type already zeroizes on drop, while the final BIP32 extended private key fields are wiped explicitly after serialization.
