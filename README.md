...
Generate a BIP39 seed phrase from physical dice rolls, hardened with system entropy.

seedroller combines the randomness of real-world dice rolls with your operating system's cryptographically secure random number generator (CSPRNG) to produce a standard 24-word BIP39 mnemonic seed phrase for Bitcoin and other wallets.
...
3. **Mix entropy sources** — Your dice rolls are concatenated with 32 bytes (256 bits) from the system CSPRNG (`OsRng`).
...
If either check fails, the program exits with an error explaining the problem. You can always enter more than 100 rolls to push the entropy above the threshold.
...
Press keys **1–6** as you roll your die — each keypress registers immediately, no Enter needed. **Backspace** undoes the last roll. After 100 rolls, press **Enter** to finish, or keep adding more rolls for extra entropy.
...
Skips the system CSPRNG. The seed is derived **only** from your dice rolls, so the same rolls always produce the same seed phrase. A bold warning is displayed when this mode is active. **Do not use this for real funds.**
...
- All sensitive intermediate values (dice rolls, system entropy, hash input, hash output) are zeroed from memory after use via [`zeroize`](https://crates.io/crates/zeroize).
